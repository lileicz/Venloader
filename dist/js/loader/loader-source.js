...
(function(ns){
	...
	var Module = function(uri,deps,factory,status){
		...
		this.status = status||STATUS.START;
	};
	Module.prototype = {
		/**
		 * 编译模块
		...
		compile : function(circleUrl){
			...
			if(ret){
				...
				var args = util.map(deps, function(uri) {
					...
					if(util.indexOf(circleUrl,uri)>-1){
						cachedModules[uri].exports;
					}else{
						circleUrl.push(uri);
						return uri ? cachedModules[uri].compile(circleUrl) : null;
					}
				});
	...
	var L = (function(){
		...
		return {
				...
				if(util.isString(uri)){
					...
					t._load(uri, function(){
						...
						deps = util.filter(deps,function(dep){
							dep = util.resolve(dep);
							return util.indexOf(circleUrl,dep)>-1?false:true;
						});
						...
						if(deps && deps.length>0){
							t.load(deps, callback, circleUrl);
						}else{
							callback();
						}
					});
					...
					for(var i=0;i<len;i++){
						(function(uri){
							t.load(uri,commonCallback,circleUrl);
						})(uri[i]);
					}
				}
			...
			_load : function(uri,callback){
				...
				if(m.status>=STATUS.LOADED){//已经加载的不再加载直接执行回调
					callback();
					return;
				}else if(m.status>=STATUS.LOADING){//正在加载的把回调放在回调队列即可
			...
			_loadScript : function(uri,callback,cs){
				...
				node.onload = node.onerror = node.onreadystatechange = function() {
			      if (READY_STATE_RE.test(node.readyState)) {
			        ...
			        if (node.parentNode && !ns.config.debug) {
	...
	ns.use = function(ids,callback){
		var args = ns.util.makeArray(arguments),
			argsL = args.length,
			preload = ns.config.preload||[];
	    ...
	    preload = util.map(preload,function(id){
	    	return util.resolve(id);
	    });
	    
	    L.load(preload,function(){
		...
		function compile(uris){
			return util.map(uris, function(uri) {
	...
	ns.add = function(deps,factory){
		ns.ignoreModuleMeta = L.add(deps,factory);
		return ns;
	};
